...
// Local dev (serve.py) always uses the local cache/ folder -- fast, no
// network dependency, matches whatever build scripts you just ran. Any
// other origin (GitHub Pages, etc.) is "production": those big data files
// aren't in this git repo (gitignored, regenerated by the build scripts),
// so they're fetched from their own R2 bucket/Worker instead -- see
// r2/worker/worker.js and r2/upload.py, kept in sync by rerunning
// r2/run_upload.ps1 after any local rebuild.
const R2_DATA_BASE = 'https://sg-view-analysis-data-bot.thunderprintsg.workers.dev/cache/';
const IS_LOCAL_DEV = ['localhost', '127.0.0.1', ''].includes(location.hostname);
function cacheUrl(name) {
  return IS_LOCAL_DEV ? `../cache/${name}` : `${R2_DATA_BASE}${name}`;
}
...
async function loadScene(file, floor) {
  ...
  let data;
  try {
    const res = await fetch(cacheUrl(`${file}.geojson`), { cache: 'no-cache' });
  ...
  try {
    const res = await fetch(cacheUrl(`${file}_floor${floor}_view.json`), { cache: 'no-cache' });
    if (res.ok) {
...
function loadIslandCache() {
  if (!islandCachePromise) {
    islandCachePromise = fetch(cacheUrl('sg_buildings_full.geojson'), { cache: 'no-cache' }).then(r => {
...
function loadManualHeights() {
  if (!manualHeightsPromise) {
    manualHeightsPromise = Promise.all([
      fetch(cacheUrl('manual_heights.json'), { cache: 'no-cache' }).then(r => r.ok ? r.json() : { by_osm_id: {}, by_name: {} }).catch(() => ({ by_osm_id: {}, by_name: {} })),
      fetch(cacheUrl('hdb_heights.json'), { cache: 'no-cache' }).then(r => r.ok ? r.json() : null).catch(() => null),
    ]).then(([manual, hdb]) => {
...
function loadLayerCache(promiseVar, filename) {
  return fetch(cacheUrl(filename), { cache: 'no-cache' }).then(r => {
